...
const TYPE_SEJOUR_LABELS = {
  ...
  SPCTT: 'Stage de perfectionnement ',
};
...
// State management using React hooks.
  ...
  // MODIFICATION ICI : On récupère tout et on filtre sur "SOUMISE" ou "DELIBERATION_CS"
  const loadListe = useCallback(async () => {
    setListLoading(true);
    setListError(null);
    try {
      // 1. On récupère la liste complète (sans filtre dans l'appel API)
      const raw = await api.demandes.list();
      const toutesLesDemandes = normalizeList(raw);
      
      // 2. On filtre pour garder les deux statuts voulus
      const demandesFiltrees = toutesLesDemandes.filter(d => 
        ["SOUMISE", "DELIBERATION_CS"].includes(d.statut)
      );

      setList(demandesFiltrees);
    } catch (e) {
...
// Side effect hook for handling data or state updates.
  ...
  const handleChangeStatusToDeliberation = async () => {
    if (!demande?.id) return;
    setStatusChangeLoading(true);
    setDetailError(null);
    try {
      await api.demandes.updateStatus(demande.id, 'DELIBERATION_CS');
      setDemande({ ...demande, statut: 'DELIBERATION_CS' });
    } catch (e) {
      setDetailError(e.message || 'Impossible de changer le statut.');
    } finally {
      ...
      <div className="ed-main">
        <header className="ed-header">
          <h1 className="ed-header-title">
            {/* Titre mis à jour */}
            {step === 'list' ? 'Examen des demandes — Soumises & Délibération' : 'Examen de dossier'}
          </h1>
            ...
            <>
              ...
              {!listLoading && !listError && list.length === 0 && (
                <div className="ed-card ed-empty">
                  {/* Texte vide mis à jour */}
                  <p className="ed-empty-title">Aucune demande en attente</p>
                  <p className="ed-muted">Les dossiers au statut « Soumise » ou « Délibération CS » apparaîtront ici.</p>
                </div>
              )}
                    ...
                    >
                      ...
                      <div className="ed-list-main">
                        ...
                        <p className="ed-list-sub">
                          {d.destination || '—'} · {typeSejourLabel(d.type_sejour.code)}
                        </p>
                      </div>
                      
                      {/* Petit extra : affichage du statut de la demande pour les différencier */}
                      <div style={{ marginRight: '15px', fontSize: '0.8rem', color: '#6b7280', fontWeight: '500' }}>
                        {d.statut === 'SOUMISE' ? 'SOUMISE' : 'EN DÉLIBÉRATION'}
                      </div>
              ...
              {!detailLoading && demande && (
                <>
                  <div className="ed-card">
                    ...
                    <div className="ed-chercheur-meta">
                      <div>
                        ...
                        <p className="ed-meta-value">{typeSejourLabel(demande.type_sejour?.code)}</p>
                      </div>
                      <div>
                  ...
                  {demande.statut === 'SOUMISE' && (
                    <div className="ed-card">
                      <h3 className="ed-section-title">Changement de statut</h3>
                      <p className="ed-status-current">
                        Statut actuel : <strong>{demande.statut}</strong>
                      </p>
                      <p className="ed-status-desc">
                        Changez le statut vers "Délibération CS" pour pouvoir approuver ou rejeter cette demande.
                      </p>
                      <button
                        type="button"
                        className="ed-status-btn"
                        onClick={handleChangeStatusToDeliberation}
                        disabled={statusChangeLoading}
                      >
                        {statusChangeLoading ? 'Changement en cours…' : 'Passer en délibération CS'}
                        {!statusChangeLoading && <IconArrowRight />}
                      </button>
                    </div>
                  )}
                  ...
                  {demande.statut === 'DELIBERATION_CS' && (
